...
int mode = 2;
int proj_mode = 0; // 0: perspective, 1: orthographic
...
void display() {
    ...
    if (proj_mode == 0) {
        ...
        float scale = 1.5f;
        projection = glm::ortho(-scale * aspect, scale * aspect, -scale, scale, 0.10f, 100.0f);
    }
...
void keyboard(unsigned char key, int x, int y) {
    if (key == '1') mode = 1;
    if (key == '2') mode = 2;
    if (key == 'p' || key == 'P') proj_mode = (proj_mode + 1) % 2; // Alterna projeção
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Cubo 3D - Perspectiva vs Ortogonal");
